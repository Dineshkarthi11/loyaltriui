...
import UserCountInput from "../UserLimitExceeded/UserCountInput";
...
function ActionButtons({ setIsModaltwoOpen }) { // Accept setIsModaltwoOpen as a prop
  const navigate = useNavigate(); // Initialize navigate hook
  const [isModalthreeOpen, setIsModalthreeOpen] = useState(false);

  const handleApplyClick = () => {
    setIsModalthreeOpen(true); // Open the modal for payment
  };
  ...
  return (
    <div className="flex gap-3 items-center mt-4 w-[410px]">
      {/* Request button */}
      <button
        onClick={() => setIsModaltwoOpen(true)} // This will open the modal if needed
