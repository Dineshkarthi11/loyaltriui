...
import TeamImg from "../../assets/images/discover/Myteammembers.png";
import EmployeeQuickViewModal from "../Employee/EmployeeQuickViewModal";
import localStorageData from "../common/Functions/localStorageKeyValues";

export default function ReportingTeamMemberCard() {
  ...
  const getTeams = async () => {
    const result = await action(API.DASHBOARD_REPORTIG_EMPLOYEE_TEAM_MEMBERS, {
      employeeId: employeeId,
      companyId: companyId,
    });
    // console.log(result,"team members data");
    setTeams(
  ...
  return (
    <DragCard
      ...
      imageIcon={TeamImg}
      header={t("My Team Members")}
      className="h-full"
    >
      {teams?.length > 0 ? (
        <div className="flex flex-col gap-5 overflow-auto max-h-60 2xl:max-h-64 px-1.5">
          {teams?.map((data, i) => (
