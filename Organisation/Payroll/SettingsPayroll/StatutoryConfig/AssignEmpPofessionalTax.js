...
export default function AssignEmpPofessionalTax({
  open,
  close = () => {},
  updateId,
  refresh = () => {},
  ...
  const [employeeCheckedListPt, setEmployeeCheckedListPt] = useState([]);
  ...
  const saveEmployee = async () => {
    ...
    try {
      const result = await Payrollaction(PAYROLLAPI.ASSIGN_EMPLOYEE_FOR_ESI, {
        ...
        companyId: companyId,
        employeeId: employeeCheckedListPt
  ...
  return (
    <DrawerPop
      ...
      header={[
        !updateId ? t("Assign Employee Tax") : t("Update Assignies"),
        !updateId
          ? t("Manage Assign Employee Tax")
